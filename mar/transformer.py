...
# https://github.com/jack139/ETH_predictor_keras_unet/blob/master/vit_unet/transformer.py
...
class ScaledDotProductAttention():
    ...
    def __call__(self, q, k, v, mask):   # mask_k or mask_qk
        ...
        attn = Lambda(lambda x: K.batch_dot(x[0], x[1], axes=[
                      2, 2])/x[2])([q, k, temper])  # shape=(batch, q, k)
        if mask is not None:
...
class SelfAttention():
    def __init__(self, d_model, d_inner_hid, n_head, layers=6, dropout=0.1):
        self.layers = [EncoderLayer(
            d_model, d_inner_hid, n_head, dropout) for _ in range(layers)]
...
# vit class and positional embeding
